...
import GrantDecisionAction from "../components/GrantDecisionAction";
import { DEFAULT_PAGESIZE } from "@app/configs/app.config";
import { IMeta } from "@app/interfaces/common.interface";
import { useEffect, useState } from "react";
import { getGrantDecision, metaGrantDecision } from "../store/GrantDecision.action";
...
export default function Page() {
    ...
    const [data, setData] = useState<GrantDecision[]>([]);
    ...
    const columns: TableProps['columns'] = [
        {
            ...
            render: (value: any, record: GrantDecision) => {
                return (
                    <span
    ...
    const fetchData = async (page: number, pageSize: number, filter: any) => {
        try {
            setIsLoading(true);
            const res = await Promise.all([getGrantDecision({ limit: pageSize, page }, filter), metaGrantDecision(filter)]);
            if (res[1]) {
    ...
    const handleViewDetail = (item: GrantDecision) => {
        openModal(
            <GrantDecisionAction detail={item} action={Action.View} />,
            {
                width: '40vw',
                onModalClose(res) {
                    if (res?.success) {
                        openModal(
                            <GrantDecisionAction detail={item} action={Action.Update} />,
                            {
                                width: '40vw',
                                onModalClose() {
                                    reloadPage();
    ...
    const handleActions = (key: Action, item: GrantDecision) => {
        console.log(key)
        if (key === Action.View) {
    ...
    return (
        <div className="w-[900px]">
            <BaseTable
                ...
                handleCreate={() => {
                    openModal(
                        <GrantDecisionAction action={Action.Create} />,
                        {
                            width: '40vw',
                            onModalClose(res) {
                                if (res?.success) {
