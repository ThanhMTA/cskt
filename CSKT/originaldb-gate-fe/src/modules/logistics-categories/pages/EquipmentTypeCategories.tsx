...
import EquipmentTypeAction from "../components/EquipmentTypeAction";
import { DEFAULT_PAGESIZE } from "@app/configs/app.config";
import { IMeta } from "@app/interfaces/common.interface";
import { useEffect, useState } from "react";
import { getEquipmentType, metaEquipmentType } from "../store/EquipmentType.action";
...
export default function Page() {
    ...
    const columns: TableProps['columns'] = [
        {
            ...
            render: (value: any, record: EquipmentType) => {
                return (
                    <span
    ...
    const fetchData = async (page: number, pageSize: number, filter: any) => {
        try {
            setIsLoading(true);
            const res = await Promise.all([getEquipmentType({ limit: pageSize, page }, filter), metaEquipmentType(filter)]);
            if (res[1]) {
    ...
    const handleViewDetail = (item: EquipmentType) => {
        openModal(
            <EquipmentTypeAction detail={item} action={Action.View} />,
            {
                width: '40vw',
                onModalClose(res) {
                    if (res?.success) {
                        openModal(
                            <EquipmentTypeAction detail={item} action={Action.Update} />,
                            {
                                width: '40vw',
                                onModalClose() {
                                    reloadPage();
    ...
    const handleActions = (key: Action, item: EquipmentType) => {
        console.log(key)
        if (key === Action.View) {
    ...
    return <div className="w-[900px]">
        <BaseTable
            ...
            handleCreate={() => {
                openModal(
                    <EquipmentTypeAction action={Action.Create} />,
                    {
                        width: '40vw',
                        onModalClose(res) {
                            if (res?.success) {
