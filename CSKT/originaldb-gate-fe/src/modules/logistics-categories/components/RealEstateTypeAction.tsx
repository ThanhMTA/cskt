...
import { updateRealEstateType, createRealEstateType, removeRealEstateType } from "../store/RealEstateType.action";
import ItemComponent from "@app/components/ItemsComponent";

type Props = {
    detail?: RealEstateType,
    action: Action
}

export default function RealEstateTypeAction({
    detail,
    action }: Props) {
    ...
    const onFinish = async () => {
        try {
            loading.show()
            const value: RealEstateType = form.getFieldsValue();
            switch (action) {
                case Action.Update:
                    await updateRealEstateType(detail?.id, value);
                    openMessage({
                        type: "success",
                    ...
                    break;
                case Action.Create:
                    await createRealEstateType(value);
                    openMessage({
                        type: "success",
    ...
    const handleDelete = async () => {
        try {
            await removeRealEstateType(detail?.id);
            openMessage({
                type: "success",
    ...
    return (
        ...
        >
            ...
            >
                <ModalCategoryActionHeader name="loại nhà" action={action} hanldeFinish={hanldeFinish} handleDelete={handleDelete} />
            </Flex>
            <Form
                form={form}
