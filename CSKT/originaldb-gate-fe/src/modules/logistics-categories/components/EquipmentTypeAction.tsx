...
import { updateEquipmentType, createEquipmentType, removeEquipmentType } from "../store/EquipmentType.action";
...
export default function EquipmentTypeAction({ detail, action }: Props) {
    ...
    const onFinish = async () => {
        try {
            loading.show()
            const value: EquipmentType = form.getFieldsValue();
            switch (action) {
                case Action.Update:
                    await updateEquipmentType(detail?.id, value);
                    openMessage({
                        type: "success",
                    ...
                    break;
                case Action.Create:
                    await createEquipmentType(value);
                    openMessage({
                        type: "success",
    ...
    const handleDelete = async () => {
        try {
            await removeEquipmentType(detail?.id);
            openMessage({
                type: "success",
    ...
    return (
        ...
        >
            ...
            >
                <ModalCategoryActionHeader name="loại vật chất" action={action} hanldeFinish={hanldeFinish} handleDelete={handleDelete} />
            </Flex>
            <Form
                form={form}
