...
import OpticalFiberLinesAction from "../components/OpticalFiberLinesAction";
import {
...
export default function OpticalFiberLines() {
  // variables
  ...
  const handleViewDetail = (item: OpticalFiberLinesData) => {
    openModal(<OpticalFiberLinesAction detail={item} action={Action.View} />, {
      width: "40vw",
      onModalClose(res) {
        if (res?.success) {
          openModal(
            <OpticalFiberLinesAction detail={item} action={Action.Update} />,
            {
              width: "40vw",
              onModalClose(res) {
                if (res?.success) {
  ...
  return (
    <div className="w-[900px]">
      <BaseTable
        ...
        btnCreate={true}
        handleCreate={() => {
          openModal(<OpticalFiberLinesAction action={Action.Create} />, {
            width: "40vw",
            onModalClose(res) {
              if (res?.success) {
                reloadData();
