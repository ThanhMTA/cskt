...
export default function Page() {
    ...
    const fetchData = async (page: number, pageSize: number, filter: any) => {
        try {
            setIsLoading(true);
            const res = await Promise.all([getRegion({ limit: pageSize, page }, filter), metaRegion(filter)]);
            if (res[1]) {
    ...
    const handleViewDetail = (item: Region) => {
        openModal(
            <RegionAction detail={item} action={Action.View} />,
            {
                width: '40vw',
                onModalClose(res) {
                    if (res?.success) {
                        openModal(
                            <RegionAction detail={item} action={Action.Update} />,
                            {
                                width: '40vw',
                                onModalClose(res) {
                                    if (res?.success) {
    ...
    const handleActions = (key: Action, item: Region) => {
        console.log(key)
        if (key === Action.View) {
    ...
    return <div className="w-[900px]">
        <BaseTable
            ...
            handleCreate={() => {
                openModal(
                    <RegionAction action={Action.Create} />,
                    {
                        width: '40vw',
                        onModalClose(res) {
                            if (res?.success) {
