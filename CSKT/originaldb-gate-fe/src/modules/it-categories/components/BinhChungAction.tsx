...
import { createBC, removeBC, updateBC } from "../store/BinhChung.action";
...
const BinhChungAction = ({ detail, action }: Props) => {
  ...
  const onFinish = async () => {
    try {
      loading.show();
      const value: BC = form.getFieldsValue();

      switch (action) {
        case Action.Update:
          await updateBC(detail?.id, value);
          openMessage({
            type: "success",
...
export default BinhChungAction;
