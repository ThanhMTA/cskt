...
import { updateAdministrativeUnit, createAdministrativeUnit, removeAdministrativeUnit } from "../store/AdministrativeUnit.action";
import ItemComponent from "../../../components/ItemsComponent";
type Props = {
    detail?: AdministrativeUnit,
    action: Action
}
export default function AdministrativeUnitAction({ detail, action }: Props) {
    ...
    const onFinish = async () => {
        try {
            loading.show()
            const value: AdministrativeUnit = form.getFieldsValue();
            switch (action) {
                case Action.Update:
                    await updateAdministrativeUnit(detail?.id, value);
                    openMessage({
                        type: "success",
                    ...
                    break;
                case Action.Create:
                    await createAdministrativeUnit(value);
                    openMessage({
                        type: "success",
    ...
    const handleDelete = async () => {
        try {
            await removeAdministrativeUnit(detail?.id);
            openMessage({
                type: "success",
        ...
        () => [
            {
                ...
                dataIndex: "name",
                rules: [
                    { required: true, message: `Hãy nhập thông tin cho trường tên đơn vị hành chính` },
                ],
            },
            {
                title: "Tên viết tắt",
    ...
    return (
        ...
        >
            ...
            >
                <ModalCategoryActionHeader name="đơn vị hành chính" action={action} hanldeFinish={hanldeFinish} handleDelete={handleDelete} />
            </Flex>
            <Form
                form={form}
