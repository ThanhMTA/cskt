...
import { getNhomTBKTList, metaNhomTBKT } from "../store/NhomTBKT.action";
import { SPECIES_FIELD_NAME, SPECIES_SCOPE_OPTIONS } from "../constants/SpeciesCategories.constant";
import  NhomTBKTAction from "../components/NhomTBKTAction";
...
export default function NhomTBKT() {
  // variables
  const { openModal } = useModal();
  const [dataSource, setDataSource] = useState<Partial< NhomTBKTData[]>>([]);
  ...
  const COLUMNS: TableProps["columns"] = [

    {
      ...
      render: (value: any, record:  NhomTBKTData) => {
        return (
          <span
  ...
  const fetchData = async (
    ...
    try {
      ...
      const response: any = await Promise.all(
        [
          await getNhomTBKTList({ limit: pageSize, page }, filter),
          await metaNhomTBKT(filter)
        ]
      );
  ...
  const handleActions = (key: Action, item:  NhomTBKTData) => {
    if (key === Action.View) {
      handleViewDetail(item);
    }
  }

  const handleViewDetail = (item:  NhomTBKTData) => {
    openModal(
      <NhomTBKTAction detail={item} action={Action.View} />,
      {
        width: '40vw',
        onModalClose(res) {
          if (res?.success) {
            openModal(
              <NhomTBKTAction detail={item} action={Action.Update} />,
              {
                width: '40vw',
                onModalClose() {
                  reloadPage();
  ...
  return (
    <>
      <BaseTable
        ...
        handleCreate={() => {
          openModal(
            < NhomTBKTAction action={Action.Create} />,
            {
              width: '40vw',
              onModalClose(res) {
                if (res?.success) {
