...
import { getHangSXList, metaHangSX } from "../store/HangSX.action";
import { SPECIES_FIELD_NAME, SPECIES_SCOPE_OPTIONS } from "../constants/SpeciesCategories.constant";
import  HangSXAction from "../components/HangSXAction";
...
export default function HangSX() {
  // variables
  const { openModal } = useModal();
  const [dataSource, setDataSource] = useState<Partial< HangSXData[]>>([]);
  ...
  const COLUMNS: TableProps["columns"] = [

    {
      ...
      render: (value: any, record:  HangSXData) => {
        return (
          <span
  ...
  const fetchData = async (
    ...
    try {
      ...
      const response: any = await Promise.all(
        [
          await getHangSXList({ limit: pageSize, page }, filter),
          await metaHangSX(filter)
        ]
      );
  ...
  const handleActions = (key: Action, item:  HangSXData) => {
    if (key === Action.View) {
      handleViewDetail(item);
    }
  }

  const handleViewDetail = (item:  HangSXData) => {
    openModal(
      <HangSXAction detail={item} action={Action.View} />,
      {
        width: '40vw',
        onModalClose(res) {
          if (res?.success) {
            openModal(
              <HangSXAction detail={item} action={Action.Update} />,
              {
                width: '40vw',
                onModalClose() {
                  reloadPage();
  ...
  return (
    <>
      <BaseTable
        ...
        handleCreate={() => {
          openModal(
            < HangSXAction action={Action.Create} />,
            {
              width: '40vw',
              onModalClose(res) {
                if (res?.success) {
