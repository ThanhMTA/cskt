...
import { getNguonDTList, metaNguonDT } from "../store/NguonDT.action";
import { SPECIES_FIELD_NAME, SPECIES_SCOPE_OPTIONS } from "../constants/SpeciesCategories.constant";
import  NguonDTAction from "../components/NguonDTAction";
...
export default function NguonDT() {
  // variables
  const { openModal } = useModal();
  const [dataSource, setDataSource] = useState<Partial< NguonDTData[]>>([]);
  ...
  const COLUMNS: TableProps["columns"] = [

    {
      ...
      render: (value: any, record:  NguonDTData) => {
        return (
          <span
  ...
  const fetchData = async (
    ...
    try {
      ...
      const response: any = await Promise.all(
        [
          await getNguonDTList({ limit: pageSize, page }, filter),
          await metaNguonDT(filter)
        ]
      );
  ...
  const handleActions = (key: Action, item:  NguonDTData) => {
    if (key === Action.View) {
      handleViewDetail(item);
    }
  }

  const handleViewDetail = (item:  NguonDTData) => {
    openModal(
      <NguonDTAction detail={item} action={Action.View} />,
      {
        width: '40vw',
        onModalClose(res) {
          if (res?.success) {
            openModal(
              <NguonDTAction detail={item} action={Action.Update} />,
              {
                width: '40vw',
                onModalClose() {
                  reloadPage();
  ...
  return (
    <>
      <BaseTable
        ...
        handleCreate={() => {
          openModal(
            < NguonDTAction action={Action.Create} />,
            {
              width: '40vw',
              onModalClose(res) {
                if (res?.success) {
