...
import { createZoneCategory, getZoneCategoryDetail, removeZoneCategory, updateZoneCategory } from "../store/ZoneCategory.action";

type ZoneCategoryActionType = {
  action: Action;
  id?: string;
};


const ZoneCategoryAction: React.FC<ZoneCategoryActionType> = ({
  action,
  id,
}) => {
  ...
  const [detail, setDetail] = useState<ZoneCategoryData | null>(null)


  const fetchDetail = async () => {
    try {
      const response = await getZoneCategoryDetail(id, {});
      setDetail(response)
    } catch (error) {
  ...
  const onFinish = async () => {
    try {
      ...
      const code = value?.code?.value ? value?.code?.value : value?.code;
      switch (action) {
        case Action.Update:
          await updateZoneCategory(detail?.id, { ...value, code });
          openMessage({
            type: "success",
          ...
          break;
        case Action.Create:
          await createZoneCategory(value);
          openMessage({
            type: "success",
  ...
  const handleDelete = async () => {
    try {
      await removeZoneCategory(detail?.id);
      openMessage({
        type: "success",
  ...
  return (
    ...
    >
      ...
      >
        <ModalCategoryActionHeader name="khu vực" action={action} hanldeFinish={hanldeFinish} handleDelete={handleDelete} />
      </Flex>
      <Form
        form={form}
...
export default ZoneCategoryAction;
