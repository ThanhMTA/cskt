...
import { createFailureCause, removeFailureCause, updateFailureCause } from "../store/FailureCauseCategories.action";
...
type Props = {
  detail?: FailureCauseCategoriesData,
  action: Action
}

const FailureCauseCategoriesAction: React.FC<Props> = ({
  detail,
  action,
}) => {
  ...
  const onFinish = async () => {
    try {
      loading.show()
      const value: FailureCauseCategoriesData = form.getFieldsValue();
      switch (action) {
        case Action.Update:
          await updateFailureCause(detail?.id, value);
          openMessage({
            type: "success",
          ...
          break;
        case Action.Create:
          await createFailureCause(value);
          openMessage({
            type: "success",
  ...
  const handleDelete = async () => {
    try {
      await removeFailureCause(detail?.id);
      openMessage({
        type: "success",
  ...
  return (
    ...
    >
      ...
      >
        <ModalCategoryActionHeader name="nguyên nhân hỏng hóc" action={action} hanldeFinish={hanldeFinish} handleDelete={handleDelete} />
      </Flex>
      <Form
        form={form}
...
export default FailureCauseCategoriesAction;
