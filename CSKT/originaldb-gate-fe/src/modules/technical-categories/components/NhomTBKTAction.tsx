...
import { createNhomTBKT, removeNhomTBKT, updateNhomTBKT } from "../store/NhomTBKT.action";
...
type NhomTBKTActionType = {
  action: Action;
  detail?: NhomTBKTData;
};
const NhomTBKTAction: React.FC<NhomTBKTActionType> = ({
  action,
  detail,
}) => {
  ...
  const onFinish = async () => {
    try {
      ...
      switch (action) {
        case Action.Update:
          await updateNhomTBKT(detail?.id, value);
          openMessage({
            type: "success",
  ...
  const handleDelete = async () => {
    try {
      await removeNhomTBKT(detail?.id);
      openMessage({
        type: "success",
