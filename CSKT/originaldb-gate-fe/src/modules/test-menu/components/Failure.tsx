...
import { getFailure } from "../stores/Failure.action";
...
const FailureManagement: React.FC = () => {
    ...
    const [filterTTB, setFilterTTB] = useState<any>({})
    ...
    const reloadData = async (record:TTBData) => {
        try {
            fetchData(pagination.page, pagination.pageSize, filter)
            try {
                // Gọi API lấy danh sách sự cố của thiết bị
                const failureList = await getFailure(record.id); // Nên trả về một mảng FailureData[]
                ...
                if (Array.isArray(failureList)) {
                    ...
                    console.warn("Dữ liệu trả về không phải mảng:", failureList);
                    setTimelineData([]);
                }
            } catch (error) {
                console.error("Lỗi khi lấy dữ liệu sự cố:", error);
                setTimelineData([]);
            }
        } catch (error: any) {
    ...
    const fetchData = async (
        ...
        try {
            ...
            const response: any = await Promise.all([
                ...
                // getFailure({ limit: pageSize, page },filterTTB),
    ...
    const showDrawer = async (record: TTBData) => {
        setDrawer(true);
    ...
    const onClose = () => {
        setDrawer(false);
        // console.log("kiem tra data-du - lieu:", timelineData)
            ...
            <Drawer
                ...
                extra={
                    <Space>
                      ...
                      <Button type="primary" 
                        onClick={() => handleActions(Action.Create)}
