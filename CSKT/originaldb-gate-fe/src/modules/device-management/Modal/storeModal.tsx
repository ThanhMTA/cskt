...
import { createTTB, removeTTB, updateTTB, getTTBDetail, getCommonCategory, getPlaceTree, getStoreTree} from "../stores/Store.action";
...
export default function StoreAction({ id, action }: Props) {
    ...
    const getDataTReeSelect = (dataIndex: string) => {
        ...
        if (["store_id"].includes(dataIndex)) {
            return selectMap(
                dataSelection?.store,
                "name",
                "id"
