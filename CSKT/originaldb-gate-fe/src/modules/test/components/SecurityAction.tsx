...
import { updateSecurity, createSecurity, removeSecurity, getSecurityDetail } from "../store/Security.action";
...
export default function SecurityAction({ id, action }: Props) {
    const [form] = Form.useForm();
    const loading = useLoading();
    const modal = useModal();
    const { openMessage } = useMessage();

    const [detail, setDetail] = useState<Security>();

    const [isLoading, setIsIoading] = useState<boolean>(false);
    const onFinish = async () => {
        try {
            loading.show()
            const value: Security = form.getFieldsValue();
            switch (action) {
                case Action.Update:
                    await updateSecurity(detail?.id, value);
                    openMessage({
                        type: "success",
    ...
    const handleDelete = async () => {
        try {
            await removeSecurity(detail?.id);
            openMessage({
                type: "success",
    ...
    const variantInput = useMemo(() => {
        return action === Action.View ? "borderless" : "outlined";
    }, [action]);

    const getDetail = async () => {
        try {
            setIsIoading(true)
            const res = await getSecurityDetail(id, {});
            console.log(res);
            setDetail(res);
            setIsIoading(false)
        } catch (e) {
    ...
    return (
        ...
        >
            ...
            >
                <ModalCategoryActionHeader name="độ mật" action={action} hanldeFinish={hanldeFinish} handleDelete={handleDelete} />
            </Flex>
