...
import { getUrgency, metaUrgency } from "../store/Urgency.action";
...
export default function Page() {
    ...
    const [data, setData] = useState<Urgency[]>([]);
    ...
    const columns: TableProps['columns'] = [
        {
            ...
            render: (value: any, record: Urgency) => {
                return (
                    <span
    ...
    const fetchData = async (page: number, pageSize: number, filter: any) => {
        try {
            setIsLoading(true);
            const res = await Promise.all([getUrgency({ limit: pageSize, page }, filter), metaUrgency(filter)]);
            if (res[1]) {
    ...
    const handleViewDetail = (item: Urgency) => {
        openModal(
            <UrgencyAction id={item?.id} action={Action.View} />,
            {
                width: '40vw',
                onModalClose(res) {
                    if (res?.success) {
                        openModal(
                            <UrgencyAction id={item?.id} action={Action.Update} />,
                            {
                                width: '40vw',
                                onModalClose() {
                                    reloadPage();
    ...
    const handleActions = (key: Action, item: Urgency) => {
        console.log(key)
        if (key === Action.View) {
    ...
    return <div className="w-[900px]">
        <BaseTable
            ...
            handleCreate={() => {
                openModal(
                    <UrgencyAction action={Action.Create} />,
                    {
                        width: '40vw',
                        onModalClose(res) {
                            if (res?.success) {
