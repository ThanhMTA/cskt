...
import { getSecurity, metaSecurity } from "../store/Security.action";
...
export default function Page() {
    ...
    const columns: TableProps['columns'] = [
        {
            ...
            render: (value: any, record: Security) => {
                return (
                    <span
    ...
    const fetchData = async (page: number, pageSize: number, filter: any) => {
        try {
            setIsLoading(true);
            const res = await Promise.all([getSecurity({ limit: pageSize, page }, filter), metaSecurity(filter)]);
            if (res[1]) {
    ...
    const handleViewDetail = (item: Security) => {
        openModal(
            <SecurityAction id={item?.id} action={Action.View} />,
            {
                width: '40vw',
                onModalClose(res) {
                    if (res?.success) {
                        openModal(
                            <SecurityAction id={item?.id} action={Action.Update} />,
                            {
                                width: '40vw',
                                onModalClose() {
                                    reloadPage();
    ...
    const handleActions = (key: Action, item: Security) => {
        console.log(key)
        if (key === Action.View) {
    ...
    return <div className="w-[900px]">
        <BaseTable
            ...
            handleCreate={() => {
                openModal(
                    <SecurityAction action={Action.Create} />,
                    {
                        width: '40vw',
                        onModalClose(res) {
                            if (res?.success) {
