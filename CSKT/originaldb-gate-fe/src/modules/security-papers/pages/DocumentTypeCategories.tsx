...
import { getDocumentType, metaDocumentType } from "../store/DocumentType.action";
...
export default function Page() {
    ...
    const columns: TableProps['columns'] = [
        {
            ...
            render: (value: any, record: DocumentType) => {
                return (
                    <span
    ...
    const fetchData = async (page: number, pageSize: number, filter: any) => {
        try {
            setIsLoading(true);
            const res = await Promise.all([getDocumentType({ limit: pageSize, page }, filter), metaDocumentType(filter)]);
            if (res[1]) {
    ...
    const handleViewDetail = (item: DocumentType) => {
        openModal(
            <DocumentTypeAction id={item?.id} action={Action.View} />,
            {
                width: '40vw',
                onModalClose(res) {
                    if (res?.success) {
                        openModal(
                            <DocumentTypeAction id={item?.id} action={Action.Update} />,
                            {
                                width: '40vw',
                                onModalClose() {
                                    reloadPage();
    ...
    const handleActions = (key: Action, item: DocumentType) => {
        console.log(key)
        if (key === Action.View) {
    ...
    return <div className="w-[900px]">
        <BaseTable
            ...
            handleCreate={() => {
                openModal(
                    <DocumentTypeAction action={Action.Create} />,
                    {
                        width: '40vw',
                        onModalClose(res) {
                            if (res?.success) {
