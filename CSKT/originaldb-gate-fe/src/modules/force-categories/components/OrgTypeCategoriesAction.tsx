...
import { createOrgType, removeOrgType, updateOrgType } from "../store/OrgTypeCategories.action";
...
type OrgTypeCategoriesActionType = {
  action: Action;
  detail?: OrgTypeCategoriesData;
};


const OrgTypeCategoriesAction: React.FC<OrgTypeCategoriesActionType> = ({
  action,
  detail,
}) => {
  ...
  const onFinish = async () => {
    try {
      ...
      switch (action) {
        case Action.Update:
          await updateOrgType(detail?.id, value);
          openMessage({
            type: "success",
  ...
  return (
    ...
    >
      ...
      >
        <ModalCategoryActionHeader name="loại đơn vị" action={action} hanldeFinish={hanldeFinish} handleDelete={handleDelete} />
      </Flex>
      <Form
        form={form}
...
export default OrgTypeCategoriesAction;
